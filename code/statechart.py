...
class Statechart(CommonLog):
    def __init__(self, sourceString):
        ...
        self.sourceFile = Path(sourceString)
        self.sourcePlantumlFile = self.sourceFile.with_suffix('.plant')
        self.statechart = sismic.io.import_from_yaml(filepath=sourceString)
        assert isinstance(self.statechart, sismic.model.Statechart)
        self.targetDirectory = Path('results').joinpath(sourceString.parts[1])
        self.targetDirectory.mkdir(parents=True, exist_ok=True)
        self.targetFile = self.targetDirectory.joinpath(sourceString.parts[2])
        self.targetPlantumlFile = self.targetFile.with_suffix('.plant')
        self.name = self.statechart.name

        sismic.io.export_to_plantuml(self.statechart, self.sourcePlantumlFile)
        subprocess.call(['java', '-jar', 'plantuml.jar', self.sourcePlantumlFile])
